...
if [ "$1" = "master" ]; then
   ...
   if [ "$2" = "2.7" ]; then
     fpm -s python --python-install-lib "/usr/lib/python2.7/dist-packages" -t deb -a all -v 0dev ./setup.py
   elif [ "$2" = "3.4" ]; then
     sudo apt-get install python3-setuptools
     sudo apt-get install -y python3-pip
     fpm -s python --python-install-lib "/usr/lib/python3/dist-packages" --python-pip /usr/bin/pip3 --python-bin /usr/bin/python3 --python-package-name-prefix python3 -t deb -a all -v 0dev ./setup.py
   fi
fi